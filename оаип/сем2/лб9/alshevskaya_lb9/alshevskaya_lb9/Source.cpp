...
void deleteX(int X, Address** phead) // Функция для удаления первого встреченного элемента с заданным номером
{
	int check_number = 0; // Счётчик для проверки номера элемента
	...
	while (current != nullptr) { 
		if (check_number == X) { 
			...
			if (current->next != nullptr) { 
				current->next->prev = current->prev; // Обновить указатель на предыдущий элемент следующего узла
			}
			if (current->prev != nullptr) { 
				current->prev->next = current->next; // Обновить указатель на следующий элемент предыдущего узла
			}
	...
	cout << "Такого номера или элемента нет" << endl;
