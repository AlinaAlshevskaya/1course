...
int main() {
    ...
    do {
        ...
        cout << " 3 - Поиск суммы отрицательных двузначных чисел" << endl;
...
// поиск суммы отрицательных двузначных чисел
void getSum(list* p) {
    int sum = 0, counter_neg = 0;
    while (p != nullptr) {
        if (p->number > -100 && p->number < -9) {
            sum += p->number;
            counter_neg++;
        }
        p = p->next;
    ...
    if (counter_neg == 0) {
        cout << "В списке нет отрицательных двузначных чисел" << endl;
    }
    else {
        cout << "Сумма отрциательных двузначных чисел: " << sum << endl;
    }
}
// Удаление числа
